...
void Socks5ConnectState::handleSetAsNewState()
{
    ...
    if (_request->addressType() == Socks5RequestMessage::IPv4 ||
            _request->addressType() == Socks5RequestMessage::IPv6 ||
                _request->addressType() == Socks5RequestMessage::DomainName)
    {

        //qDebug() << "diy-socks server ip" << _parent->remoteIp->addresses().first().toString();
        ...
        _socket->connectToHost(_parent->remoteIp->addresses().first(), _parent->remotePort);
...
{
    qDebug() <<"remote server connected";
    ...
    qDebug() <<"sock info:" << _socket->localAddress().toString() << "  port:" << _socket->localPort();
